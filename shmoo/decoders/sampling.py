...
class SamplingDecoder(Decoder):
    ...
    def __init__(self, config):
        ...
        self.seed = config.get('decoder_config', {}).get(
            'seed', utils.DEFAULT_SEED)
        self.num_samples = config.get('decoder_config', {}).get(
            'num_samples', utils.DEFAULT_NUM_SAMPLES)

    def process(
            self, input_features: Dict[str, Any]) -> Sequence[Dict[str, Any]]:
        hypos = [self.make_initial_hypothesis(input_features) for i in
