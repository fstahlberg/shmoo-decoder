...
class SamplingDecoder(Decoder):
    ...
    def __init__(self, config):
        ...
        self.seed = utils.get_from_decoder_config(config, 'seed', DEFAULT_SEED)
        self.num_samples = utils.get_from_decoder_config(
            config, 'num_samples', DEFAULT_NUM_SAMPLES)
        self.strategy = utils.get_from_decoder_config(
            config, 'strategy', DEFAULT_SAMPLING_STRATEGY)

        if self.strategy == 'temperature':
            self.temp = utils.get_from_decoder_config(
                config, 'temperature', DEFAULT_TEMPERATURE)
            self.make_probs = partial(self.normalise_scores, temp=self.temp)
        elif self.strategy == 'top k':
            self.k = utils.get_from_decoder_config(config, 'k', DEFAULT_TOP_K)
            self.make_probs = partial(self.normalise_top_k_scores, k=self.k)
        elif self.strategy == 'nucleus':
            self.cutoff_nucleus_p = utils.get_from_decoder_config(
                config, 'p', DEFAULT_NUCLEUS_P)
            self.make_probs = partial(
                self.normalise_scores_acc_to_prob_mass,
                cutoff_p=self.cutoff_nucleus_p)
        elif self.strategy == 'typical':
            self.cutoff_typical_p = utils.get_from_decoder_config(
                config, 'typical p', DEFAULT_TYPICAL_P)
            self.make_probs = partial(
                self.normalise_scores_acc_to_expected_info,
                cutoff_p=self.cutoff_typical_p)
        else:
    ...
    def normalise_scores(self, log_probs: np.ndarray,
                         temp: float) -> np.ndarray:
        return np.exp(log_probs) if temp == 1 else softmax(log_probs / temp)

    def normalise_top_k_scores(self, log_probs: np.ndarray,
                               k: int) -> np.ndarray:
    ...
    def _normalise_scores_acc_to_prob_mass(self, log_probs: np.ndarray,
        ...
        crit_ind = (np.cumsum(sorted_probs) < cutoff_p - np.finfo(
            np.float32).eps).argmin()
        sparse_probs = np.zeros(log_probs.size)
        sparse_probs[flat_indices[:crit_ind + 1]] = np.exp(
            log_probs[flat_indices[:crit_ind + 1]])
        return sparse_probs / np.sum(sparse_probs)

    def normalise_scores_acc_to_prob_mass(self, log_probs: np.ndarray,
                                          cutoff_p: float) -> np.ndarray:
    ...
    def process(
        ...
        hypos = [self.make_initial_hypothesis(input_features) for _ in
        ...
        while not self._finished_criterion(hypos):
            ...
            hypos = [self.make_hypothesis(prediction, lazy=True) for prediction
                     in predictions]
